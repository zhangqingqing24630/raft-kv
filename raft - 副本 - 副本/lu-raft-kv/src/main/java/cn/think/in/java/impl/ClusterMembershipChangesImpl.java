...
public class ClusterMembershipChangesImpl implements ClusterMembershipChanges {
    ...
    public RpcClient rpcClient = NettyClient.getInstance();
    /**
     * 必须是同步的,一次只能添加一个节点
     *
     * @param newPeer
     */
    @Override
    public synchronized Result addPeer(Peer newPeer) throws ExecutionException, InterruptedException, TimeoutException {
        //leader向新节点同步日志
...
//            node.peerSet.getList().add(newPeer);
            ...
            long size=node.logModule.getLastIndex();
            int sizee=(int)size;
            if(sizee>0) {
                CountDownLatch countDownLatch = new CountDownLatch(sizee);
