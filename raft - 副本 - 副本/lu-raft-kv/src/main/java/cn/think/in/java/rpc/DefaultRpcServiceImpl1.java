 ...
 */
@Slf4j
public class DefaultRpcServiceImpl1 implements RpcService {

    private final DefaultNode node;

    private final RpcServer rpcServer;

    public DefaultRpcServiceImpl1(int port, DefaultNode node) {
        rpcServer = new RpcServer(port, false, false);
        rpcServer.registerUserProcessor(new RaftUserProcessor1<Request>() {

            @Override
            public Object handleRequest(BizContext bizCtx, Request request) throws ExecutionException, InterruptedException, TimeoutException {
    ...
    @Override
    public Response<?> handlerRequest(Request request) throws ExecutionException, InterruptedException, TimeoutException {
        ...
        } else if (request.getCmd() == Request.CHANGE_PEERSET_ADD) {
            return new Response<>((node).addPeerSet((PeerSet) request.getObj()));
        }
        return null;
    }
