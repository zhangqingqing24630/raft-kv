...
public class RaftNodeBootStrap {
    ...
    public static void boot() throws Throwable {
        ...
        if (StringUtil.isNullOrEmpty(property)) {
            //bug 当peerAddr只有一个时，下线了再上线就会单独成一个集群,因为再也没有办法找到原集群
            peerAddr = new String[]{"localhost:8776","localhost:8777"};
            //peerAddr = new String[]{"localhost:8776","localhost:8777","localhost:8778","localhost:8779","localhost:8780"};
        } else {
