...
//处理PUT请求，然后将用户的 KV 数据封装成日志结构，包括 term，index，command，预提交到本地
        ...
        for (Peer peer : peerSet.getPeersWithOutSelf()) {
            ...
            count++;
            // 并行发起 RPC 日志复制.
            //向每一个节点发起同步日志的请求，限时20s，过期返回false
        ...
        // Leader更新commitIndex需要计算大多数节点拥有的日志范围，
        // 通过对matchIndex排序找中位数的index，就是大多数节点都拥有的日志范围，将其设置为commitIndex。
        ...
        if (N > commitIndex) {
            ...
            //System.out.println(entry);//不会更新前一任期的日志
            ...
            for (Future future : futureArrayList) {
                RaftThreadPool.submit(() -> {
                    try {
                        ...
                        //System.out.println(response.getResult().getTerm());
